<script lang="ts">
  ...
  $: if (featureManager.isEnabled('clientSideRAG') && !isInitialized) {
    initializeRAG();
  }

  // Reactive statement to reload documents when lastQuery changes (indicating new activity)
  $: if (lastQuery && isInitialized) {
    loadDocuments();
  }

  // Reactive statement to refresh when forceRefresh counter changes
  $: if (forceRefresh > 0 && isInitialized) {
    loadDocuments();
  }

  let refreshInterval: ReturnType<typeof setInterval> | null = null;

  // Periodically refresh documents when panel is visible
  $: {
    if (isVisible && isInitialized) {
      if (!refreshInterval) {
        refreshInterval = setInterval(loadDocuments, 2000);
      }
    } else {
      if (refreshInterval) {
...
{#if featureManager.isEnabled('clientSideRAG') && isVisible}
  <div
    class="rag-context-panel bg-surface-100-800-token border-l border-surface-300-600-token p-4 w-80 overflow-y-auto"
  >
    ...
    {:else}
      ...
      <!-- RAG Settings -->
      <div class="border-t border-surface-300-600-token pt-4 mb-4">
      ...
      <div class="border-t border-surface-300-600-token pt-4">
        <h4 class="font-medium mb-2 text-sm">Features</h4>
